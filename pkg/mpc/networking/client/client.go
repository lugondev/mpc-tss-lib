...
func (s *Client) doOperation(ctx context.Context, operation []byte, tssOperation common.OperationFunc) ([]byte, error) {
	...
	if err != nil {
		...
		_ = conn.Close(fmt.Errorf("client error: %w", err))
		return nil, err
	}
...
func (s *Client) doOperation_(
	...
	go func() { errCh <- common.OpError{Type: "res", Err: s.receiver(conn, inCh)} }()     // server -> Tss
	go func() { errCh <- common.OpError{Type: "tra", Err: s.transmitter(conn, outCh)} }() // Tss -> server
