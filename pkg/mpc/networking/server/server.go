...
func (s *Server) doOperation_(
	...
	go func() {
		...
		errCh <- common.OpError{Type: "tss", Err: err}
		tssWaiter <- nil
	}()
	go func() { errCh <- common.OpError{Type: "res", Err: s.receiver(outCh)} }()
	go func() { errCh <- common.OpError{Type: "tra", Err: s.transmitter(outCh, inCh)} }()
	...
	for {
		...
		case err := <-errCh:
			...
			if err.Type == "res" {
				...
				if err := checkResults(s.results, ownResult, s.Tss == nil); err != nil {
...
func checkResults(results map[string][]byte, ownResult []byte, isServer bool) error {
	for clientID, v := range results {
		if !bytes.Equal(v, ownResult) && !isServer {
			fmt.Println("results", hexutil.Encode(ownResult))
