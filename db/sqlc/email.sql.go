...
const getEmailForContract = `-- name: GetEmailForContract :many
SELECT id, email_id, contract_id, created_at
FROM emails_contract
WHERE contract_id = $1
`

func (q *Queries) GetEmailForContract(ctx context.Context, contractID int64) ([]EmailsContract, error) {
	rows, err := q.db.QueryContext(ctx, getEmailForContract, contractID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []EmailsContract{}
	for rows.Next() {
		var i EmailsContract
		if err := rows.Scan(
			&i.ID,
			&i.EmailID,
			&i.ContractID,
